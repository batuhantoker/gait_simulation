...
def plot_slip_trajectory(slip_traj: Union[(SlipTrajectory, SlipGaitCycleCtrl)], color='k', plot_passive=True,
    ...
    gait=np.array([0,0,0,0,0,0])
    foot_pos_list = np.array([0,0])
    contact = np.array([0])
    time =  np.array([0])
    time_label =  np.array([0])
    ...
    for gait_cycle in cycles:
        ...
        contact = np.append(contact,gait_cycle.foot_contact_pos)
        time = np.hstack((time,gait_cycle.t_flight,gait_cycle.t_stance))
        for i in time:
            if i in gait_cycle.t_flight and i not in gait_cycle.t_stance:
                time_label = np.append(time_label,0)
            if i in gait_cycle.t_stance and i not in gait_cycle.t_flight :
                time_label = np.append(time_label,1)




                                            
        gait=np.vstack((gait,gait_cycle.flight_cartesian_traj.T,gait_cycle.stance_cartesian_traj.T))
    ...
    for index,i in enumerate(time_label):
        ...
        if i == 1:
            foot = np.array([contact[a+1],0])
            contact_prev = 0
            k=0
        if i == 0:

            next_contact = [x-index-1 for x in np.where(time_label[index:] == 1)][0][0]
            if next_contact > contact_prev:
        ...
        foot_pos_list  = np.vstack((foot_pos_list ,foot))
        i_prev = i
